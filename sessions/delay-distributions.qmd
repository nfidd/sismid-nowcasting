...
In this session we will use the `nfidd.nowcasting` package to load a data set of infection times and access stan models and helper functions, the `ggplot2` package for plotting, the `dplyr` and `tidyr` packages to wrangle data, the `lubridate` package to deal with dates, the `posterior` and `tidybayes` packages for investigating the results of the inference conducted with stan.
...
We will work with a data set that is included in the `nfidd.nowcasting` R package that you installed initially.
...
Instead of copying these between sessions, we've put them into functions in the `nfidd.nowcasting` R package that we can use when needed.
...
A prior is the probability distribution we assign to a parameter before we see the data.
It says what values of the parameter we think are plausible, and Bayes' rule combines it with the likelihood to give the posterior.

We like priors because they let us include domain knowledge in a model, such as the fact that a delay from onset to hospitalisation is a few days rather than a few years.
They also stabilise estimates when the data are sparse, where the likelihood on its own says little about the parameter.

All methods have some kind of prior, it just may not be stated explicitly.
A frequentist maximum likelihood estimate corresponds to the most likely parameter values under a Bayesian model with a flat prior, i.e. one that treats every parameter value, however extreme, as equally plausible.
Writing the prior down makes that assumption visible and lets us choose something better.

For more on this see the [Stan reference guide](../reference/stan).
:::
