...
### What is the downside of using Stan directly? {.smaller}
...
- A random walk on the delay parameters with priors
...
<svg viewBox="0 0 880 410" width="100%" style="max-width:820px;font-family:inherit;" role="img" aria-label="Five epinowcast modules — expectation, reference date, report date, missing data and observation — combine into one nowcast.">
  <defs>
  ...
  <g stroke="#5b6b7b" stroke-width="2" fill="none" marker-end="url(#cr-arrow)">
    <path d="M322,44 C450,44 520,190 600,201"/>
    <path d="M322,124 C440,124 500,198 600,204"/>
    <path d="M322,204 C440,204 520,204 600,204"/>
    <path d="M322,284 C440,284 500,210 600,207"/>
    <path d="M322,364 C450,364 520,218 600,210"/>
  </g>
  <g text-anchor="middle">
    ...
    <text x="172" y="280" font-size="16" font-weight="600" fill="#1d2b3a">Missing data</text>
    <text x="172" y="301" font-size="13.5" font-family="monospace" fill="#41586e">enw_missing()</text>
    <rect x="22" y="335" width="300" height="58" rx="9" fill="#eef2f7" stroke="#33506e" stroke-width="1.5"/>
    <text x="172" y="360" font-size="16" font-weight="600" fill="#1d2b3a">Observation</text>
    <text x="172" y="381" font-size="13.5" font-family="monospace" fill="#41586e">enw_obs()</text>
    <rect x="600" y="154" width="220" height="100" rx="10" fill="#2c7fb8" stroke="#1d5a86" stroke-width="1.5"/>
    <text x="710" y="198" font-size="19" font-weight="600" fill="#ffffff">Nowcast</text>
    <text x="710" y="222" font-size="13.5" fill="#dbeaf5">counts + reporting</text>
  </g>
</svg>
</div>
```

The **expectation**, **reference** and **observation** modules mirror what we built by hand.
The **report date** module is new and handles effects on the report date, such as day-of-week reporting.
The **missing data** module is also new and handles reports with an unknown reference date.

### Mapping back to the bespoke model {.smaller}

| Bespoke model | `epinowcast` module |
|---|---|
| $\lambda_{t}$ random walk | <code>enw_expectation(~ 0 + (1 &#124; day))</code> |
...
::: {.callout-note}
`epinowcast` is one tool among several.
Models written using Stan directly, `baselinenowcast` benchmarks, and other frameworks all have their place.
