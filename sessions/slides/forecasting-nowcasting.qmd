...
- But forecast uncertainty grows with the horizon, so longer forecasts are harder
...
### A taste of forecasting {.smaller}

A first look at forecasting - the [forecasting course](https://nfidd.github.io/sismid-forecasting/) covers it in full.

- **Forecasting**: a probabilistic statement about dates with *no data yet* (the future)
- Good forecasts are **probabilistic**; uncertainty grows with the horizon
- Our renewal model is already *generative*: keep simulating $R_t$ forward to forecast
- We **evaluate** forecasts against what was observed, using proper scoring rules (e.g. CRPS)
...
2. Implement 4 forecasting approaches (complete data, two pipelines, joint)
3. Compare performance quantitatively with a proper scoring rule (CRPS)
