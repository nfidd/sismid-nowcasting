...
The CRPS is a proper scoring rule that generalises the mean absolute error to probabilistic forecasts, so lower is better, and it rewards forecasts that are sharp subject to being calibrated.
Its breakdown splits the score into *overprediction* and *underprediction* (the cost of predicting too high or too low) and *dispersion* (the cost of a forecast that is too wide or too narrow).
The companion [forecasting course](https://nfidd.github.io/sismid-forecasting/sessions/forecast-evaluation) covers the CRPS and these components in depth.
