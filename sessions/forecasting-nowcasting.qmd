...
Throughout this course we have focused on **nowcasting**: estimating what current data will look like once all delayed reports are in.
In this final session we take a first **taste of forecasting** - predicting future trends - and see how naturally it extends the nowcasting and renewal-equation models you have already built.

This is the bridge to the companion [forecasting course](https://nfidd.github.io/sismid-forecasting/), which runs in the second half of the week and covers forecasting, forecast evaluation, ensembles, and forecast hubs in much more depth.
Here we give just enough of a taste to motivate that course and to show why nowcasting and forecasting belong together.

In real-time epidemic analysis nowcasting and forecasting are connected.
When we want to forecast from the most recent data, we face the problem that this data is incomplete due to reporting delays - exactly the problem nowcasting is designed to solve.
...
This session aims to give a taste of forecasting and to show how to combine nowcasting and forecasting to make better predictions when data are subject to reporting delays.

## What is forecasting?

- **Forecasting** means making a quantitative, probabilistic statement about what will happen on dates for which we have *no data at all yet* (the future).
- A good forecast is **probabilistic**: rather than a single number it gives a full distribution, so it expresses how uncertain we are. Forecast uncertainty grows the further ahead we look.
- We have, in fact, already done most of the work needed to forecast. The renewal-equation model from the [$R_t$ estimation session](R-estimation-and-the-renewal-equation) is a *generative* model: once it has estimated the recent reproduction number, we can keep simulating the random walk for $R_t$ forward in time to generate infections, and therefore onsets, on future days. That extension is exactly what turns an estimation model into a forecasting model, and the Stan model we use below (`estimate-inf-and-r-rw-forecast`) does precisely this.
- To know whether a forecast was any good we **evaluate** it by comparing the predicted distribution against what was eventually observed, using a *proper scoring rule* (we use the Continuous Ranked Probability Score, CRPS, below).

That is the entire forecasting toolkit we need here. The [forecasting course](https://nfidd.github.io/sismid-forecasting/) develops each of these ideas - generative and time-series forecasting models, scoring rules, ensembles, and hubs - in full.
...
In this session we will use the `nfidd.nowcasting` package to load a data set of infection times and access Stan models and helper functions, the `dplyr` and `tidyr` packages for data wrangling, `ggplot2` library for plotting, the `tidybayes` package for extracting results of the inference, and `scoringutils` for forecast evaluation.
...
As we've seen throughout this course, epidemiological data often arrives with delays.
...
Because forecast uncertainty grows with the forecast horizon, starting from older data means even our forecasts of "what is happening now" might end up being quite uncertain.
...
This builds on the approach from the [joint nowcasting session](joint-nowcasting), but now we'll also create a dataset filtered at a two-week forecast horizon for evaluation (i.e. the data that we would ultimately observe for the two weeks after the forecast date).

::: {.callout-note collapse="true"}
## Beyond simulated onsets: other surveillance data

We use simulated line-list data here (individual symptom onsets with reporting delays) because it lets us see the whole infection → onset → report process clearly, and because it connects directly to the renewal and nowcasting models we have already built.

Real surveillance streams behave differently in ways worth keeping in mind:
...
**Other surveillance data (e.g. compound indicators such as influenza-like illness, ILI):**

- Aggregated, compound measures (e.g. % of visits for ILI) rather than individual events
- Can be revised up *or* down as later reports arrive
- May combine multiple diseases and reporting locations

The methods in this session adapt to these settings: the joint modelling framework can handle bidirectional revisions with minor modifications, while pipeline approaches may need separate models for upward and downward revisions.
Whether dealing with simple delays or more complex revisions, combining nowcasting and forecasting improves predictions.

The companion [forecasting course](https://nfidd.github.io/sismid-forecasting/) works with real surveillance data of this kind and explores these issues in depth.
:::
...
For the more complex models we might also consider using approximate fitting methods such as variational inference or integrated nested Laplace approximations.
:::

::: {.callout-tip}
## Take 2 minutes

How does this model differ from the one we used in the [Rt estimation session](R-estimation-and-the-renewal-equation)?
...
- The model is the same as the one we used in the [Rt estimation session](R-estimation-and-the-renewal-equation), but with an additional block where we generate the forecast for additional days beyond the observation period. 
...
- **Forecast starts 14 days before cutoff**: The approach has to start forecasting from day 57 (cutoff - 14) to reach day 71 (cutoff), creating a longer forecast horizon
...
- **Delay distribution estimation**: Methods from the [biases in delay distributions session](biases-in-delay-distributions) to estimate reporting delays from individual-level data
- **Simple nowcasting**: The geometric random walk nowcasting model from the [nowcasting concepts session](nowcasting) 
...
- **Reproduction number estimation**: Methods from the [R estimation session](R-estimation-and-the-renewal-equation) applied to the nowcasted data
...
We start by fitting a geometric random walk nowcast to the reporting triangle data, as we did in the [joint nowcasting session](joint-nowcasting).
...
- The geometric random walk and renewal equation represent different generative processes [@lisonGenerativeBayesianModeling2024], which can lead to inconsistencies as we saw in the [joint nowcasting session](joint-nowcasting).
...
::: {.callout-note}
## Scoring forecasts with `scoringutils`

The `score()` output reports the CRPS along with a breakdown of it.

- A **proper scoring rule** scores a probabilistic forecast against the eventual observation; a forecaster minimises their expected score by reporting what they really believe, so honest forecasts are rewarded rather than ones tuned to game the metric.
  The CRPS is *strictly* proper, so reporting your true belief is the **only** way to minimise the expected score.
- The **Continuous Ranked Probability Score (CRPS)** generalises the mean absolute error to full probabilistic forecasts: it measures the gap between the forecast's cumulative distribution and a step at the observed value. Lower is better, it is in the same units as the data, and for a point forecast it reduces to the absolute error.
- Guiding principle: as **sharp** as possible while staying **calibrated**.
- The CRPS breaks down into *overprediction* / *underprediction* (predicting too high or too low) and *dispersion* (a forecast that is too wide).
  A forecast that is too narrow is not penalised through dispersion; it misses the observation, so it shows up as over- or underprediction instead.
- The companion [forecasting course](https://nfidd.github.io/sismid-forecasting/sessions/forecast-evaluation) develops the CRPS, proper scoring rules and these components in depth.
:::

::: {.callout-tip .optional collapse="true"}
## Interactive: the CRPS

Drag the observation or widen the forecast and watch the area — and the score — change. (From the companion [forecasting course](https://nfidd.github.io/sismid-forecasting/sessions/forecast-evaluation).)

```{=html}
<iframe class="nfidd-embed" src="https://nfidd.github.io/sismid-forecasting/interactive/forecast-evaluation.html?embed=crps" title="CRPS interactive" loading="lazy" style="width:100%;height:640px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
...
In practice, implementations often include hierarchical models for multiple locations, time-varying delays, and ensemble approaches.
