...
In this session we will use the `nfidd.nowcasting` package to load a data set of infection times and access stan models and helper functions, the `dplyr` and `tidyr` packages for data wrangling, `ggplot2` library for plotting, and the `tidybayes` package for extracting results of the inference.
...
As we discussed in the [session on biases in delay distributions](biases-in-delay-distributions), this can be done using individual data and then passing this estimate to a simple nowcasting model.
...
In the [nowcasting concepts session](nowcasting) we also saw that getting the delay distribution wrong can lead to very poor nowcasts.
...
Recall from the [nowcasting concepts session](nowcasting) that we simulated individual reporting delays and then aggregated them. 
Now we'll simulate the same process but at the population level with observation error - for the same reasons we made this change in the [convolutions session](using-delay-distributions-to-model-the-data-generating-process-of-an-epidemic).
...
Finally, we sum this to get the counts we actually observe. This is the same as the date we corrected for **right truncation** in the [nowcasting concepts session](nowcasting).
...
This is just a **reformulation** of the nowcasting problem we saw in the [nowcasting concepts session](nowcasting).

:::

This richer data structure contains information about both the delay distribution and the final expected counts so we can use it to jointly estimate both, which was not possible with the simpler data structure we used in the [nowcasting concepts session](nowcasting).
...
Try and do this using methods from [session on biases in delay distributions](biases-in-delay-distributions) and see how it affects the simple nowcast models.
