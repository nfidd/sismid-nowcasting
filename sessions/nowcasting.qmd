...
## Interactive: scaling up by the proportion reported

```{=html}
<iframe class="nfidd-embed" src="../interactive/nowcasting.html?embed=simple" title="the simplest nowcast interactive" loading="lazy" style="width:100%;height:780px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
...
Read off the proportion of a reference date reported by now, the delay CDF at the "now" line, and scale the observed count back up by it.
:::
...
In the [session on biases in delay distributions](biases-in-delay-distributions) we saw how we could do this using individual-level records.
