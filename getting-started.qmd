...
This is the **nowcasting** course, which forms the first half of the [SISMID](https://sismid.sph.emory.edu/) week.
A companion [forecasting course](https://nfidd.github.io/sismid-forecasting/) covers the second half of the week.
...
2.  Install the `nfidd.nowcasting` R package
3.  Install cmdstan
...
#### Installation of the `nfidd.nowcasting` package

To install the packages needed in the course, including the `nfidd.nowcasting` package that contains data files and helper functions used throughout:

```{r install, eval=FALSE}
if (!require("pak")) {
    install.packages("pak")
}
pak::pak("nfidd/sismid-nowcasting", dependencies = TRUE)
```
...
remotes::install_github("nfidd/sismid-nowcasting", dependencies = TRUE)
```
:::
...
`cmdstan` is compiled from source, so it needs a working C++ toolchain.
On Windows you will most likely need to install [Rtools](https://cran.r-project.org/bin/windows/Rtools/) before the step below will work, matching the version of Rtools to your version of R.
:::

Installing `cmdstan` is a separate step:
...
-   Directly download the course material:
    ...
    ::: callout-tip
    [[**Download**]{.underline}](https://github.com/nfidd/sismid-nowcasting/archive/refs/heads/main.zip)
    :::

-   Alternatively, if you are familiar with git you can clone the [repo](https://github.com/nfidd/sismid-nowcasting).
...
::: callout-important
## Run the setup chunk in each session

Each session starts with a "Setup" callout that is collapsed by default, so you have to click on it to open it.
Inside it are the code chunks that load the packages used in the session and set the random seed, along with a note of where the source `.qmd` file lives.
Run these chunks before anything else in the session.
If you skip them the later code will fail because the packages are not loaded, and your results will not match ours because the seed has not been set.
:::
...
If you're returning to the course after some time or joining a live session, you need to update your setup so that you have the latest content and package versions.

### Quick Update (Recommended)

1.  **Update the nfidd.nowcasting package** (this is quick and ensures you have the latest functions):
...
2.  **Download fresh course materials** if using local files:
    ...
    -   Download the latest version: [[**Download**]{.underline}](https://github.com/nfidd/sismid-nowcasting/archive/refs/heads/main.zip)
